...
// Selecting a main file inside a folder must still return a PDF and resolve BibTeX.
const nestedFiles = files.map((file) => ({ ...file, path: file.path === 'main.tex' ? 'paper/article.tex' : file.path }))
const nestedResult = await compilePaper(nestedFiles, new AbortController().signal, progress, factory, 180000, 'paper/article.tex')
assert.ok(nestedResult.pdf.length > 1000)
assert.doesNotMatch(nestedResult.log, /Citation .+ undefined|Reference .+ undefined/)
...
assert.equal(terminated, 5, 'Every successful, failed, timed out or cancelled job terminates its worker')
